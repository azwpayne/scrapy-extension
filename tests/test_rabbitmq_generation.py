...
def test_connect_closes_candidate_on_baseexception_in_publish_window(mocker) -> None:
    ...
    mocker.patch.object(
        backend, "_publish_handles_locked", side_effect=KeyboardInterrupt
    )
...
def test_cluster_host_diagnostic_failure_keeps_connect_live(
    ...
    if has_extra_nodes:
        ...
        assert backend.is_connected() is True
        assert (
            logger_debug.call_count == 2
        )  # cluster-count + published-session diagnostics
    else:
...
def test_qos_success_logger_failure_keeps_prepared_candidate_live(
    ...
    candidate_channel.basic_qos.assert_called_once_with(
        prefetch_count=1, prefetch_size=0
    )
    assert backend._connection is candidate_connection
    assert backend._channel is candidate_channel
    assert backend.is_connected() is True
