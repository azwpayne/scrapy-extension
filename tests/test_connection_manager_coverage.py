...
def test_T8_close_non_last_holder_is_noop_on_backend():
    ...
    b = ConnectionManager.get_manager(
        "redis", {"k": 8}
    )  # same key -> _users=2, same mgr
    assert a is b
    a._backend = fake
...
def test_T10_backend_property_owner_error_propagates_to_all_waiters():
    ...
    def worker():
        ...
        try:
            _ = (
                m.backend
            )  # property access triggers connect (raises); assign silences ruff B018
    ...
    deadline = time.monotonic() + 5
    while (fake.connect_calls < 1 or past_barrier < 3) and time.monotonic() < deadline:
        time.sleep(0.01)
