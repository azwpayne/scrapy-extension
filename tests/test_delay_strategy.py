...
class TestDelayQueueStrategy:
    ...
    def test_pop_with_ack_preserves_token_after_local_deadline(
        ...
        delegated = mocker.patch.object(
            strat, "_pop_backend_with_ack", side_effect=backend_pop
        )
    ...
    def test_pop_with_ack_threads_mq_token(
        self, mock_connection_manager, mocker
    ) -> None:
    ...
    def test_drain_retains_priority_kwarg_form(self, mock_connection_manager) -> None:
        ...
        assert (
            call.kwargs.get("priority", call.args[2] if len(call.args) > 2 else 0.0)
            == 7.5
        )
    ...
    def test_failed_drain_keeps_due_item_for_retry(
        self, mock_connection_manager
    ) -> None:
...
class TestSnapshotRestore:
    ...
    def test_fallback_diagnostic_failure_preserves_completed_delay_state(
        ...
        if site == "soft-cap":
            ...
            mocker.patch.object(
                delay_module.logger, "warning", side_effect=diagnostic_error
            )
