...
def test_registry_normalization_handles_cycles_instance_state_and_slots() -> None:
    ...
    assert "cycle" in repr(connectors_module._normalize_registry_value(cyclic, set()))
...
def test_scheduler_explicit_ack_and_nack_fail_closed_without_queue() -> None:
    ...
    scheduler._ack_request_token(
        Request("https://example.com/no-token"), log_message="ack"
    )
    scheduler._nack_request_token(
        Request("https://example.com/no-token"), log_message="nack"
    )
...
def test_download_failure_wrapper_settles_sync_and_async_streams() -> None:
    ...
    async def run_async_paths() -> None:
        ...
        assert [
            value async for value in wrapper._finish_success(async_request, values())
        ] == [1]
        ...
        with pytest.raises(RuntimeError, match="async stream failed"):
            _ = [
                value async for value in wrapper._finish_success(async_source, broken())
            ]
...
def test_dupefilter_interrupted_decision_compensation_is_best_effort(
    monkeypatch,
) -> None:
    dupefilter = _new_dupefilter()
    owner = object()
...
def test_dupefilter_monitor_fallback_cleanup_preserves_process_signal() -> None:
    ...
    active_requests = FailCleanupGet({id(request): (ref(request), active_tokens)})
    dupefilter = _new_dupefilter()
...
def test_dupefilter_factory_wraps_constructor_and_cleanup_failures(
    ...
    with pytest.raises(ConfigurationError, match="Invalid SCRAPY_DEDUP_MEMORY_MAXSIZE"):
        BackendDupeFilter.from_settings(Settings({"SCRAPY_DEDUP_STRATEGY": "memory"}))
...
def test_queue_body_codec_and_callback_validation_fail_closed() -> None:
    with pytest.raises(
        SerializationError, match="Unsupported queued request body codec"
    ):
...
def test_queue_strategy_defensive_defaults_and_local_durability_gate() -> None:
    ...
    strategy = MagicMock(spec=QueueStrategy)
    assert (
        QueueStrategy.is_push_durable(strategy, delay=0.0, source="coverage") is False
    )
