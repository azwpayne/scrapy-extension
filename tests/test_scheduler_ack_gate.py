...
class TestAckCapabilityGate:
    ...
    def test_G11_synthetic_single_slot_with_concurrency_1_skips_gate(
        self, mocker
    ) -> None:
...
def _strategy_settings(
    backend_type: str, strategy: str, *, concurrent: int = 16
) -> Mock:
...
class TestStrategyMqAckBypassWarning:
    ...
    def test_no_warn_when_delay_threads_ack_with_kafka(self, mocker, caplog) -> None:
        ...
        with caplog.at_level(
            logging.WARNING, logger="scrapy_extension.schedule.scheduler"
        ):
    ...
    def test_no_warn_when_passthrough_strategy_pairs_with_kafka(
        self, mocker, caplog
    ) -> None:
        import logging
    ...
    def test_no_warn_when_delay_strategy_pairs_with_atomic_redis(
        self, mocker, caplog
    ) -> None:
        import logging
