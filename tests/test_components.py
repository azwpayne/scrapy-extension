...
class TestBackendScheduler:
    ...
    def test_enqueue_request_returns_false_for_duplicate(
    ...
    def test_enqueue_request_serialization_error_returns_false_and_increments_stats(
        ...
        scheduler.open(spider)
        mock_connection_manager.get_queue_backend().push.side_effect = (
            SerializationError("bad request payload")
        )
    ...
    def test_enqueue_does_not_touch_set_backend(
    ...
    def test_enqueue_request_set_backend_not_implemented(
        self, mock_connection_manager, mock_spider
    ):
    ...
    def test_next_request_deserialization_error(self, mock_connection_manager):
        ...
        mock_connection_manager.get_queue_backend().pop.side_effect = (
            SerializationError("bad request payload")
        )
    ...
    def test_close_calls_connection_manager_close(
        self, mock_connection_manager, mock_spider
    ):
    ...
    def test_close_resets_signals_connected_for_reuse(
        self, mock_connection_manager, mocker
    ):
    ...
    def test_on_response_received_noop_when_queue_none(
        self, mock_connection_manager, mocker
    ):
    ...
    def test_on_response_received_swallows_queue_error(
        self, mock_connection_manager, mocker
    ):
        ...
        scheduler._on_response_received(
            response=None, request=request, spider=mock_spider
        )
        ack_spy.assert_called_once_with(token="tok")

    def test_on_spider_error_noop_when_queue_none(
        self, mock_connection_manager, mocker
    ):
    ...
    def test_on_spider_error_swallows_queue_error(
        self, mock_connection_manager, mocker
    ):
...
class TestSchedulerAckTokenFlow:
    ...
    def test_on_response_received_forwards_token_to_ack(self, mocker):
        ...
        scheduler._on_response_received(
            mock_response, mock_request, spider=mocker.MagicMock()
        )
