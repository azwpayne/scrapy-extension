...
def test_rabbitmq_backend_warns_when_ssl_disabled(mocker, caplog):
    ...
    assert any(
        "loopback" in rec.message and "plaintext" in rec.message
...
def test_rabbitmq_backend_ssl_warning_debounces_across_reconnects(mocker, caplog):
    ...
    ssl_warnings = [
        rec
        for rec in caplog.records
        if "loopback" in rec.message and "plaintext" in rec.message
    ]
...
def test_rabbitmq_backend_mirrored_queues_amqp_error(mocker):
    ...
    mock_channel.exchange_declare.side_effect = pika.exceptions.AMQPError(
        "Policy error"
    )
...
def test_rabbitmq_backend_disconnect_connection_amqp_error(mocker):
    ...
    backend.connect()
    mock_instance.close.side_effect = pika.exceptions.AMQPError(
