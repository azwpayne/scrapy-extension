    ...
    [
        ...
        (
            "cluster_startup_nodes",
            {
                ...
                "cluster_startup_nodes": [
                    f"redis://user:{_SECRET}@cluster.internal:6379"
                ],
            },
        ),
    ],
...
def test_cluster_scalar_ipv6_fallback_reaches_sdk_without_brackets(mocker) -> None:
    ...
    assert [(node.host, node.port) for node in startup_nodes] == [("2001:db8::1", 6380)]
...
def test_ghost_masters_field_is_rejected_without_retaining_endpoint(
    ...
    with pytest.raises(ConfigurationError) as exc_info:
        RedisSettings(**{field_name: [f"user:{_SECRET}@master.internal:6379"]})
...
def test_cluster_redirect_budget_is_instance_local_and_plus_one(mocker) -> None:
    ...
    first = RedisBackend(RedisSettings(mode=RedisMode.CLUSTER, cluster_max_redirects=0))
    second = RedisBackend(
...
_OPERATION_CONTRACTS: tuple[tuple[str, type[BaseException], str, str], ...] = (
...
def test_cluster_sdk_exception_obeys_every_operation_contract(
    ...
    mocker.patch("scrapy_extension.backends.redis.RedisCluster", return_value=client)
