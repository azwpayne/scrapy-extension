...
class TestPipelinePerComponentBackend:
    ...
    def test_uses_storage_backend_type_when_set(self, mock_settings, mocker):
        ...
        settings = mock_settings(
            ...
            getdicts={
                "SCRAPY_STORAGE_BACKEND_SETTINGS": {"uri": "mongodb://storage:27017"}
            },
        )
        _, patched = _patch_get_manager(mocker)
    ...
    def test_from_crawler_threads_storage_backend_type(self, mock_settings, mocker):
...
class TestResolveBackendConfig:
    ...
    def test_per_component_type_without_settings_returns_empty_dict(
        self, mock_settings
    ):
    ...
    def test_invalid_backend_type_raises_configuration_error(self, mock_settings):
        ...
        settings = mock_settings(
            gets={"SCRAPY_QUEUE_BACKEND_TYPE": "not-a-real-backend"}
        )
