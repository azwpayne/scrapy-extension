...
def test_path_vs_string_do_not_collide():
    ...
    key_path = ConnectionManager._registry_key(
        BackendType.MONGODB, {"cert": Path("/a")}
    )
...
def test_mixed_type_mapping_keys_are_stable_without_collision():
    ...
    key_a = ConnectionManager._registry_key(
        BackendType.REDIS,
        {"a": "x", 2: "y"},  # type: ignore[arg-type]
    )
    key_b = ConnectionManager._registry_key(
        BackendType.REDIS,
        {"a": "x", 2: "z"},  # type: ignore[arg-type]
    )
    assert key_a != key_b
