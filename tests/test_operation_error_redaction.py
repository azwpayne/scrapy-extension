...
def _connected_sqs_backend(mocker: Any) -> tuple[SqsBackend, MagicMock]:
  backend = SqsBackend(
    SqsSettings(endpoint_url=f"http://{_MARKER}.example:4566")
  )
  client = mocker.MagicMock()
...
def test_pulsar_pop_rebuilds_driver_error_without_topic_or_config_state(mocker: Any) -> None:
...
def test_breaker_proxy_rebuilds_backend_errors_and_open_error_without_bound_method() -> None:
...
def test_forwarded_proxy_uses_lookup_fallback_for_nonweakrefable_plugin_callable() -> None:
...
def test_forwarded_wrapper_returns_static_error_when_weak_backend_is_unavailable() -> None:
