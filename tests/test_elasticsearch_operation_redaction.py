...
def _failing_set_api_operation(
  mocker: Any, method_name: str
) -> Callable[[], object]:
  backend, client = _backend(mocker)
...
def test_elasticsearch_boundaries_validate_inputs_before_backend_work(mocker: Any) -> None:
  backend, client = _backend(mocker)
...
def test_elasticsearch_expired_document_warning_omits_storage_key(mocker: Any) -> None:
  ...
  client.get.return_value = {
    "_source": {
      "data": "ZGF0YQ==",
      "expireAt": (datetime.now(tz=timezone.utc) - timedelta(seconds=1)).isoformat(),
    }
  }
