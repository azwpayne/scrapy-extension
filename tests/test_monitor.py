...
class TestBackendQueueMonitorWiring:
    ...
    def test_wiring_resolves_monitor_from_spider_crawler(
...
class TestDupeFilterMonitorWiring:
    ...
    def test_request_seen_emits_hit_on_duplicate(self, mock_connection_manager):
        monitor = ScrapyStatsMonitor(_stats())
    ...
    def test_request_seen_emits_miss_on_new(self, mock_connection_manager):
        monitor = ScrapyStatsMonitor(_stats())
        df = BackendDupeFilter(
            connection_manager=mock_connection_manager,
            monitor=monitor,
...
class TestPopRateEmission:
    ...
    def test_pop_rate_not_emitted_before_sampling_window(self, mock_connection_manager):
...
class TestFilterSaturationEmission:
    ...
    def test_request_seen_emits_saturation_for_cuckoo(self, mock_connection_manager):
...
class TestR14DObservability:
    ...
    def test_on_error_emitted_on_pop_deserialize_failure(self, mock_connection_manager):
    ...
    def test_request_seen_emits_saturation_for_bloom(self, mock_connection_manager):
    ...
    def test_memory_filter_at_cap_emits_saturation(self, mock_connection_manager):
    ...
    def test_raising_on_disconnect_monitor_does_not_skip_breaker_reset(self, mocker):
        from scrapy_extension.backends.connectors import ConnectionManager
    ...
    def test_on_pop_attempt_stat_name_matches_behavior(self, mock_connection_manager):
