...
class TestSqsBackendType:
    ...
    def test_visibility_timeout_accepts_supported_boundaries(
        self, timeout: int
    ) -> None:
...
class TestSqsConnect:
    ...
    def test_private_session_client_failure_remains_unpublished(self, mocker) -> None:
        b = _make_backend()
    ...
    def test_connect_creates_client(self, mocker) -> None:
        b = _make_backend()
        client = mocker.MagicMock()
        _session_factory, client_factory = _patch_client(mocker, return_value=client)
        b.connect()
    ...
    @pytest.mark.parametrize("region_name", ["us-gov-west-1", "eusc-de-east-1"])
    ...
    def test_slow_queue_resolution_does_not_block_other_queue_ack(self, mocker) -> None:
        b = _make_backend()
        client = mocker.MagicMock()
...
class TestSqsPushPop:
    ...
    def test_push_maps_colon_logical_name_to_stable_aws_name(self, mocker) -> None:
        ...
        assert all(
            character.isalnum() or character in "-_" for character in physical_name
        )
    ...
    def test_push_ignores_priority(self, mocker) -> None:
        ...
        assert b._client.send_message.call_args.kwargs.keys() >= {
            "QueueUrl",
            "MessageBody",
        }
    ...
    def test_pop_returns_decoded_bytes(self, mocker) -> None:
        ...
        client.receive_message.return_value = {
            "Messages": [
                {"Body": base64.b64encode(b"hello").decode(), "ReceiptHandle": "rh"}
            ]
        }
...
class TestSqsAckNack:
    def test_ack_deletes_message(self, mocker) -> None:
        ...
        client.receive_message.return_value = {
            "Messages": [
                {"Body": base64.b64encode(b"x").decode(), "ReceiptHandle": "rh"}
            ]
        }
        b.pop("queue1")
        b.ack("queue1")
    ...
    def test_nack_makes_message_immediately_visible_and_clears_receipt(
        self, mocker
    ) -> None:
        import base64
...
class TestSqsLenClear:
    ...
    def test_same_queue_push_waits_for_the_purge_barrier(self, mocker) -> None:
        ...
        mocker.patch(
            "scrapy_extension.backends.sqs.time.sleep", side_effect=blocking_sleep
        )
        mocker.patch.object(
            b,
...
class TestSqsAckCorrectQueueUrl:
    ...
    def test_ack_records_correct_url_across_queues(self, mocker) -> None:
        ...
        client.receive_message.side_effect = [
            {
                "Messages": [
                    {"Body": base64.b64encode(b"a").decode(), "ReceiptHandle": "rh-a"}
                ]
            },
            {
                "Messages": [
                    {"Body": base64.b64encode(b"b").decode(), "ReceiptHandle": "rh-b"}
                ]
            },
        ]
        b.pop("qA")
...
class TestSqsRealInFlightAck:
    ...
    def test_three_pops_then_three_distinct_acks(self, mocker) -> None:
        ...
        client.receive_message.side_effect = [
            {
                "Messages": [
                    {"Body": base64.b64encode(b"m1").decode(), "ReceiptHandle": "rh-1"}
                ]
            },
            {
                "Messages": [
                    {"Body": base64.b64encode(b"m2").decode(), "ReceiptHandle": "rh-2"}
                ]
            },
            {
                "Messages": [
                    {"Body": base64.b64encode(b"m3").decode(), "ReceiptHandle": "rh-3"}
                ]
            },
        ]
...
class TestSqsRealNack:
    def test_nack_with_token_requeues_immediately_and_discards_from_in_flight(
        ...
        client.receive_message.return_value = {
            "Messages": [
                {"Body": base64.b64encode(b"x").decode(), "ReceiptHandle": "rh-1"}
            ]
        }
        _, tok = b.pop_with_ack("q")
    ...
    def test_nack_failure_raises_queue_error_and_keeps_local_token(
        self, mocker
    ) -> None:
        token = _SqsAckToken("https://sqs/test", "rh-1")
        b, client = _connected(mocker)
