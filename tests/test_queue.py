...
class TestBackendQueueRequestToDict:
    ...
    def test_binary_body_round_trips_through_pop(
        self, mock_connection_manager, mock_spider
    ):
...
class TestBackendQueuePush:
    ...
    def test_push_serializes_and_calls_backend(
        self, mock_connection_manager, mock_spider
    ):
    ...
    def test_push_pop_preserves_request_subclass(self, mock_connection_manager, mocker):
    ...
    def test_nack_delegates_to_queue_backend(
        self, mock_connection_manager, mock_spider
    ):
        ...
        queue.nack()
        mock_connection_manager.get_queue_backend().nack.assert_called_once_with(
            "test_queue"
        )
    ...
    def test_push_raises_serialization_error_on_exception(
        self, mock_connection_manager, mock_spider
    ):
    ...
    def test_push_propagates_backend_queue_error(
        self, mock_connection_manager, mock_spider
    ):
        ...
        backend_error = QueueError(
            "Push failed", queue_name="test_queue", operation="push"
        )
    ...
    @pytest.mark.parametrize(
        "strategy_cls", [DelayQueueStrategy, TimeWheelQueueStrategy]
    )
    ...
    def test_local_route_is_frozen_before_serialization_side_effects(
        ...
        if isinstance(strategy, DelayQueueStrategy):
            ...
            assert (
                sum(len(slot) for slot in strategy._wheel) + len(strategy._overflow)
                == 1
            )
    ...
    def test_push_rejects_invalid_priority_without_mutating_meta(
        ...
        request = Request(
            "https://example.com", meta={"delay": 5.0, "source": "feed-a"}
        )
...
class TestBackendQueuePop:
    ...
    def test_pop_restores_callback_and_errback_with_spider(
        self, mock_connection_manager
    ):
    ...
    def test_pop_rejects_non_callable_attribute(self, mock_connection_manager, mocker):
    ...
    def test_pop_roundtrips_binary_body_without_corruption(
        self, mock_connection_manager
    ):
    ...
    def test_pop_records_poison_drop_stat(self, mock_connection_manager, mocker):
        strategy = mocker.MagicMock()
        strategy.pop_with_ack.return_value = (b"invalid json", "token-1")
        ...
        spider.crawler.stats.inc_value.assert_any_call("scheduler/queue/poison_dropped")
...
class TestBackendQueueDepthSampling:
    ...
    def test_sampling_reduces_queue_len_calls(
        self, mock_connection_manager, mock_spider
    ):
