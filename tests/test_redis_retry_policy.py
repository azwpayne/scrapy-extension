...
class _CommittedThenResponseLostSentinelConnection(SentinelManagedSSLConnection):
...
@pytest.mark.parametrize("mode", [RedisMode.STANDALONE, RedisMode.MASTER_SLAVE])
...
def test_legacy_warning_does_not_render_configuration_source() -> None:
    ...
    with warnings.catch_warnings(record=True) as records:
        ...
        RedisBackend(RedisSettings(password=secret_marker, retry_on_timeout=True))
...
def test_real_cluster_max_redirects_bounds_consecutive_moved_follow_ups(
    ...
    def real_cluster_factory(**kwargs):
        ...
        node.redis_connection.connection_pool.connection_class = _AlwaysMovedConnection
        slot = client.keyslot(b"{scrapy-extension:queue:queue}:items")
    ...
    assert exc_info.value.__context__ is None
    assert all(
        command[0] in {"EVALSHA", "EVAL"} for command in _AlwaysMovedConnection.commands
    )
