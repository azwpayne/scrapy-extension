...
| Item | Disposition rationale |
|---|---|
| **Sentinel failover re-discovery** (round-2 residual) | The round-2 framing ("no re-discovery path exists") was **imprecise**. A full `ConnectionManager` reconnect *does* re-discover the master via a fresh `_sentinel.master_for()` call (`backends/redis.py` + `connectors.py`). **Superseded 2026-07-21:** the former claim that data-plane `retry_on_timeout=True` safely provided eventual recovery was incorrect across redis-py 7.3–8.x; response-loss retries can replay committed push/pop Lua mutations. Bundled data commands now use zero automatic replay after outcome-ambiguous transport failures. Sentinel's separate setting permits at most one immediate SDK retry per read-only control request after a timeout; data-command recovery requires a later caller/manager attempt with outcome-aware reconciliation. |
