...
def validate_mongodb_uri(value: object) -> str:
    ...
    if (
        ...
        or not value
        or not value.lower().startswith(_VALID_MONGO_SCHEMES)
    ):
        raise ConfigurationError(
    ...
    option_names = {name for name, _value in _mongodb_uri_option_pairs(parsed.query)}
...
def _parse_mongodb_seed_endpoint(value: object, setting_name: str) -> tuple[str, str]:
...
def validate_mongodb_authentication(
    ...
    if username is not None and (type(username) is not str or not username.strip()):
        raise ConfigurationError(
...
def validate_mongodb_transport_security(
    *,
    ...
    if authenticated and not effective_tls and not local_standalone_plaintext:
        raise ConfigurationError(
            (
...
class MongoDBSettings(RedactedBaseSettings):
    ...
    allow_remote_plaintext: bool = Field(
        ...
        description=(
            ...
            "MongoDB deployment on a trusted private network"
        ),
    )
