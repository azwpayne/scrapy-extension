...
def _redis_address_error(
    setting_name: str, *, index: int | None = None
) -> ConfigurationError:
...
def normalize_redis_host(host: object, *, setting_name: str = "host") -> str:
    ...
    legacy_numeric_labels = all(
        label.isdigit() or re.fullmatch(r"0[xX][0-9A-Fa-f]+", label) for label in labels
    )
    if (
        not dns_name
        or len(dns_name) > 253
...
def is_redis_loopback_host(host: object) -> bool:
    ...
    """
    return is_loopback_host(normalize_redis_host(host))
...
def _redis_configured_endpoints_are_loopback(settings: RedisSettings) -> bool:
    """Return whether every configured Redis transport endpoint is local."""
    if settings.mode is RedisMode.SENTINEL:
        endpoints = tuple(settings.sentinels)
    elif settings.mode is RedisMode.CLUSTER:
        endpoints = tuple(settings.cluster_startup_nodes) or (
            format_redis_endpoint(settings.host, settings.port),
        )
    else:
        return is_redis_loopback_host(settings.host)
    return bool(endpoints) and all(
        is_redis_loopback_host(
            parse_redis_endpoint(endpoint, setting_name="transport_endpoint")[0]
        )
...
class RedisSettings(RedactedBaseSettings):
    ...
    read_from_replicas: bool = Field(
        default=False,
        description=("Deprecated unsupported replica-read input; must remain false."),
        json_schema_extra={"deprecated": True},
    )
    ...
    sentinel_retry_on_timeout: bool = Field(
        default=True,
        description=("Retry one timeout per read-only Sentinel control SDK request"),
    )
