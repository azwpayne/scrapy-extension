...
_REDIS_SAFE_CONNECTION_MESSAGES: frozenset[str] = frozenset(
    {f"Failed to connect to Redis ({mode.value})" for mode in RedisMode}
    | {
...
class RedisBackend(Backend, QueueBackend, SetBackend, StorageBackend):
    ...
    def _capture_connection_plan(
        ...
        try:
            validated = RedisSettings.model_validate(canonical_values, strict=True)
        ...
        snapshot = _RedisConnectionSnapshot(
            ...
            cluster_skip_full_coverage_check=(
                validated.cluster_skip_full_coverage_check
            ),
    ...
    def _response_decode_kwargs(snapshot: _RedisConnectionSnapshot) -> dict[str, str]:
        ...
        return (
            {"encoding_errors": "surrogateescape"} if snapshot.decode_responses else {}
        )
    ...
    def _build_and_publish_generation(
        ...
        try:
            ...
            if snapshot.mode in (RedisMode.STANDALONE, RedisMode.MASTER_SLAVE):
                ...
                client.RedisClusterRequestTTL = snapshot.cluster_max_redirects + 1
                self._raise_if_connect_cancelled(request_epoch)
            else:  # pragma: no cover - RedisSettings validation owns this branch
                raise ConfigurationError("Unsupported Redis mode.", setting_name="mode")
    ...
    def pop(self, queue_name: str, timeout: float = 0.0) -> bytes | None:
        ...
        with self._lease_generation("pop from a queue") as generation:
            ...
            if normalized_timeout == 0:
                return self._atomic_pop_once(
                    queue_name, queue_key, payload_key, pop_script
                )

            assert deadline is not None
            while True:
                item = self._atomic_pop_once(
                    queue_name, queue_key, payload_key, pop_script
                )
                if item is not None:
                    return item
    ...
    def queue_len(self, queue_name: str) -> int:
        ...
        with self._lease_generation("read a queue length") as generation:
            queue_key = self._queue_key(
                queue_name, namespace=generation.snapshot.namespace
            )
            try:
    ...
    def store(self, key: str, data: bytes, ttl: int | None = None) -> None:
        ...
        with self._lease_generation("store a value") as generation:
            storage_key = self._storage_key(
                key, namespace=generation.snapshot.namespace
            )
    ...
    def clear_storage(self, prefix: str | None = None) -> None:
        ...
        with self._lease_generation("clear storage") as generation:
            pattern = f"{self._storage_key(logical_prefix, namespace=generation.snapshot.namespace)}*"
            try:
