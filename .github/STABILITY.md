...
| `ConnectionManager.acquire_lease()` / `ConnectionManagerLease` / `release_manager_acquire()` (`scrapy_extension.backends.connectors`) | Experimental | Additive lease-scoped acquisition over the Stable `ConnectionManager` pool: each `acquire_lease()` call is released by exactly one idempotent `lease.release()`, and `release_manager_acquire(owner, exact=True)` rolls back one acquire with one safe retry while preserving the factory's primary failure (the default `exact=False` closes the shared manager). Signature and teardown semantics may evolve in a pre-1.0 minor release with changelog notice. |
| `ConnectionManager.apply_scrapy_breaker_policy()` | Experimental | Applies an explicit Scrapy `SCRAPY_CIRCUIT_BREAKER_*` policy to an already-resolved shared manager without changing pool identity; a later differing already-explicit policy is dropped with a one-shot diagnostic. Shares the Experimental breaker-settings tier below. |
...
|---|---|---|---|---|---|
| Redis | Yes | Yes | Yes | Stable — full | Namespaced queue/set/storage domains; atomic Lua queue operations; leased immutable client/namespace generations with draining teardown; no automatic data-command transport replay. Three effective topologies: standalone, Sentinel, and Cluster; `master_slave` is a deprecated primary-only compatibility alias, while its replica-read inputs are deprecated and reject unsupported intent. Cluster is DB0-only and its bounded redirect continuation budget is distinct from transport retry. The Stable `retry_on_timeout` compatibility input is deprecated and does not enable data replay. The raw `client` property is a point-in-time SDK escape hatch. Legacy unnamespaced keys require explicit migration. |
...
| ElasticSearch | Yes | Yes | Yes | Stable — full | 2 modes (standalone, cloud); immutable leased generations; configured retry policy retained for safe reads/control setup, while all data mutations use a shared-transport no-replay view. Mutation response loss is surfaced as a typed indeterminate outcome and must be reconciled by callers: no automatic replay is not exactly-once. Partial, timed-out, failed-shard, and malformed responses fail closed. |
...
| Pulsar | Yes | No | No | Stable — queue-only | Topic-bound concurrent ack tokens; per-topic background receive pumps make the first zero-time poll non-blocking, bound local prefetch to 100, recycle failed pumps only after serialized consumer retirement, and use bounded close/worker teardown waits. Unreturned buffered deliveries remain unsettled for redelivery. Queue depth and purge require an admin API and are unsupported here. |
